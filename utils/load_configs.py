...
def get_link_classification_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--dataset_name', type=str, help='dataset to be used', default='wikipedia',
                        choices=['junyi','EdnetKT1','Slepemapy', 'assist17','assisst12x','assisst12','wikipedia', 'reddit', 'mooc', 'lastfm', 'myket', 'enron', 'SocialEvo', 'uci', 'Flights', 'CanParl', 'USLegis', 'UNtrade', 'UNvote', 'Contacts'])
    ...
    parser.add_argument('--model_name', type=str, default='DyGFormer', help='name of the model, note that EdgeBank is only applicable for evaluation',
                        choices=['DyGKT','QIKT','IEKT','IPKT','DIMKT','simpleKT','AKTMemory','DKTMemory','CTNCM','AKT','DKT','JODIE', 'DyRep', 'TGAT', 'TGN', 'CAWN', 'EdgeBank', 'TCL', 'GraphMixer', 'DyGFormer'])
    ...
    parser.add_argument('--num_neighbors', type=int, default=50, help='number of neighbors to sample for each node') #this is a good one
...
def get_link_prediction_args(is_evaluation: bool = False):
    ...
    """
    # arguments
    parser = argparse.ArgumentParser('Interface for the link classification task')
    parser.add_argument('--dataset_name', type=str, help='dataset to be used', default='wikipedia',
                        choices=['Slepemapy','assist17','assisst12x','assisst12','wikipedia', 'reddit', 'mooc', 'lastfm', 'myket', 'enron', 'SocialEvo', 'uci', 'Flights', 'CanParl', 'USLegis', 'UNtrade', 'UNvote', 'Contacts'])
    parser.add_argument('--batch_size', type=int, default=200, help='batch size')
    parser.add_argument('--model_name', type=str, default='DyGFormer', help='name of the model, note that EdgeBank is only applicable for evaluation',
                        choices=['DKTMemory','DKT','JODIE', 'DyRep', 'TGAT', 'TGN', 'CAWN', 'EdgeBank', 'TCL', 'GraphMixer', 'DyGFormer'])
    parser.add_argument('--gpu', type=int, default=0, help='number of gpu to use')
...
def load_link_prediction_best_configs(args: argparse.Namespace):
    ...
    if args.model_name == 'TGAT':
        ...
        if args.dataset_name in ['wikipedia']:
            ...
            args.dropout = 0.5
        if args.dataset_name in ['mooc', 'uci', 'USLegis']:
            args.dropout = 0.4
