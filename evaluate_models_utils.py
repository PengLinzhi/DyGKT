...
def evaluate_model_link_classification(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
    ...
    #if model_name in ['AKTMemory','DKTMemory','DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'DyGFormer','DKT','AKT','CTNCM','simpleKT']:
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            #                                                                 node_interact_times=batch_node_interact_times,
            #                                                                 edge_ids=batch_edge_ids)
            elif model_name in ['AKTMemory','DKTMemory']:
            ...
            elif model_name in ['TGAT', 'CAWN', 'TCL']:
            ...
            # predicts = model[1](batch_src_node_embeddings).squeeze(dim=-1).sigmoid()
            # standard_label = torch.tensor([0,1],dtype=torch.float32,device=predicts.device)
            # labels = torch.tensor(batch_edge_labels, dtype=torch.float32,device=predicts.device)
                #print("labels.sum()",labels.sum())
                #print("predicts-labels",predicts.shape,labels.shape)
...
def evaluate_model_link_prediction(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
    ...
    with torch.no_grad():
        ...
        train_predict = torch.cat(evaluate_predicts, dim=0)
        train_label = torch.cat(evaluate_labels, dim=0)

        evaluate_metrics.append(get_link_prediction_metrics(predicts=train_predict, labels=train_label))
