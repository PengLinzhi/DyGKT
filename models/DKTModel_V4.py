...
class DKTModel(nn.Module):
    ...
    def compute_src_dst_node_temporal_embeddings(self,src_node_ids: np.ndarray, edge_ids:np.ndarray,
        ...
        src_node_embeddings = self.src_node_updater.update(
            src_nodes_features[:, :-1, :] + src_nodes_neighbor_skill_struct_features+ src_nodes_neighbor_struct_features) + src_nodes_features[:,-1, :]
        dst_node_embeddings = self.dst_node_updater.update(dst_nodes_features[:, :-1, :]+ dst_nodes_neighbor_struct_features) +\
                              dst_nodes_features[:,-1,:] #+ dst_nodes_neighbor_skill_struct_features
    ...
    def get_features(self, node_interact_times: np.ndarray, nodes_neighbor_ids: np.ndarray, nodes_edge_ids: np.ndarray,
        ...
        if self.use_node_features:
            nodes_neighbor_node_raw_features = self.projection_layer['feature'](self.node_raw_features[torch.from_numpy(nodes_neighbor_ids)].to(self.device)) # 现在做的题目本身的skill！！
        if not self.use_node_features:
            nodes_neighbor_node_raw_features = self.projection_layer['feature'](0*self.node_raw_features[torch.from_numpy(nodes_neighbor_ids)].to(self.device))
...
class DKT(nn.Module):
    ...
    def update(self, x):
        ...
        # return torch.mean(patches_data, dim=1)
...
class DKT_forgetting(nn.Module):
